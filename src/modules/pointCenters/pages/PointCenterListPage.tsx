...
export function PointCenterListPage() {
  ...
  useEffect(() => { loadPointCenters(); }, [loadPointCenters]);
    ...
    <div className="space-y-4 pb-8">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map(pc => (
            <div key={pc.id} onClick={() => navigate(`/point-centers/${pc.id}`)} className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm cursor-pointer hover:shadow-md transition flex items-start gap-3">
              ...
              <div className="flex-1">
                <div className="flex justify-between items-start">
                  ...
                  <span className={`text-[10px] px-2 py-0.5 rounded-full ${pc.active ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
                    {pc.active ? 'مستمر' : 'غير مستمر'}
                  </span>
                </div>

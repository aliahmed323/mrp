...
export function DoctorDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<'info' | 'deals' | 'visits'>('info');
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getDoctorVisits(id),
      db.orders.where('doctorId').equals(id).toArray()
    ]).then(([d, c, p, v, o]) => {
      setDoctor(d ?? null);
      setClinics(c);
      setPharmacies(p);
    ...
    <>
      <div className="space-y-4 max-w-2xl mx-auto pb-8">
        ...
        <div className="flex rounded-xl overflow-hidden border border-slate-200 p-1 bg-slate-50 gap-1 shrink-0">
          <button
            ...
            className={cn(
              ...
              activeTab === 'info' ? 'bg-white shadow-sm text-blue-700' : 'text-slate-600 hover:bg-slate-100'
            )}
          >
            المعلومات
          </button>
          <button
            ...
            className={cn(
              'flex-1 text-sm py-2.5 rounded-lg transition-colors font-bold',
              activeTab === 'deals' ? 'bg-white shadow-sm text-blue-700' : 'text-slate-600 hover:bg-slate-100'
            )}
          >
            تعاملاتي
          </button>
          <button
            ...
            className={cn(
              'flex-1 text-sm py-2.5 rounded-lg transition-colors font-bold',
              activeTab === 'visits' ? 'bg-white shadow-sm text-blue-700' : 'text-slate-600 hover:bg-slate-100'
            )}
          >
            الزيارات
          </button>
        ...
        {activeTab === 'deals' && (
          <div className="space-y-4">
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
              <h3 className="text-sm font-semibold text-slate-700 mb-4">طلبيات الطبيب المباشرة ({orders.length})</h3>
              {orders.length === 0 ? (
                <p className="text-sm text-slate-500 text-center py-4 bg-slate-50 rounded-xl">لا توجد طلبيات مسجلة لهذا الطبيب</p>
              ) : (
                <div className="space-y-3">
                  {orders.map(o => (
                    <div key={o.id} className="p-3 rounded-xl border border-slate-100 bg-slate-50">
                      <div className="flex justify-between items-start mb-2">
                        <span className="font-semibold text-sm text-slate-900">{o.productName}</span>
                        <span className="text-xs bg-white border px-2 py-0.5 rounded-full">{o.orderDate}</span>
                      </div>
                      <div className="flex justify-between items-center text-xs text-slate-500">
                        <span>الكمية: {o.quantity} {o.bonus && `+ ${o.bonus}`}</span>
                        <span>الصيدلية: {o.pharmacyName}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
              <h3 className="text-sm font-semibold text-slate-700 mb-4">إحصائيات "مرات الكتابة"</h3>
              <p className="text-xs text-slate-500 text-center py-4 bg-slate-50 rounded-xl">
                يتم حسابها من خلال المنتجات المسجلة في الزيارات الدورية لهذا الطبيب.
              </p>
            </div>
          </div>
        )}
        ...
        {activeTab === 'visits' && (
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 space-y-3">
            <h3 className="text-sm font-semibold text-slate-700 flex items-center gap-2 mb-4">
              <Activity size={16} className="text-blue-500" /> سجل الزيارات ({visits.length})
            </h3>
            {visits.length === 0 ? (
              <p className="text-xs text-slate-500 text-center py-4">لم تسجل أي زيارات بعد</p>
            ) : (
              <div className="space-y-3">
                {visits.map(v => (
                  <Link to={`/visits/${v.id}`} key={v.id} className="flex gap-3 p-3 rounded-xl border border-slate-100 hover:border-blue-200 hover:shadow-md transition-all">
                    <div className="w-10 h-10 rounded-lg bg-blue-50 flex items-center justify-center shrink-0">
                      <CalendarDays size={16} className="text-blue-600" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between">
                        <span className="font-bold text-sm text-slate-900">{v.date}</span>
